import { Request } from 'express'

import { Body, Controller, Get, Param, Post, Req } from '@nestjs/common'
import { RequestHelper } from '@server/helpers/request'
import { EventService } from '@server/libraries/event'
import { DiscussionDomainFacade } from '@server/modules/discussion/domain'
import { AuthenticationDomainFacade } from '@server/modules/authentication/domain'
import { DiscussionApplicationEvent } from './discussion.application.event'
import { DiscussionCreateDto } from './discussion.dto'

import { VideoDomainFacade } from '../../video/domain'

@Controller('/v1/videos')
export class DiscussionByVideoController {
  constructor(
    private videoDomainFacade: VideoDomainFacade,

    private discussionDomainFacade: DiscussionDomainFacade,
    private eventService: EventService,
    private authenticationDomainFacade: AuthenticationDomainFacade,
  ) {}

  @Get('/video/:videoId/discussions')
  async findManyVideoId(
    @Param('videoId') videoId: string,
    @Req() request: Request,
  ) {
    const queryOptions = RequestHelper.getQueryOptions(request)

    const parent = await this.videoDomainFacade.findOneByIdOrFail(videoId)

    const items = await this.discussionDomainFacade.findManyByVideo(
      parent,
      queryOptions,
    )

    return items
  }

  @Post('/video/:videoId/discussions')
  async createByVideoId(
    @Param('videoId') videoId: string,
    @Body() body: DiscussionCreateDto,
    @Req() request: Request,
  ) {
    const { user } = this.authenticationDomainFacade.getRequestPayload(request)

    const valuesUpdated = { ...body, videoId }

    const item = await this.discussionDomainFacade.create(valuesUpdated)

    await this.eventService.emit<DiscussionApplicationEvent.DiscussionCreated.Payload>(
      DiscussionApplicationEvent.DiscussionCreated.key,
      {
        id: item.id,
        userId: user.id,
      },
    )

    return item
  }
}
